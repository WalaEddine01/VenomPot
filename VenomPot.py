...
# NEW IMPORT:
from ssh_VenomPot import run_ssh_VenomPot
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Start SSH
        if args.ssh:
             print(f"[-] Starting SSH Honeypot on port {args.sshport}...")
             t_ssh = threading.Thread(target=run_ssh_VenomPot, args=(args.sshport, args.username, args.password))
             t_ssh.daemon = True
             t_ssh.start()
        ...
        # Keep main alive
        while True:
            # Join with a timeout to allow KeyboardInterrupt to be caught
            for t in threads:
                t.join(timeout=1.0)
